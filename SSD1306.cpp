...
SSD1306::SSD1306(int CLK_PIN, int MOSI_PIN, int RST_PIN, int DC_PIN)
{
  _CLK_PIN = CLK_PIN;
  _MOSI_PIN = MOSI_PIN;
  _RST_PIN = RST_PIN;
...
void SSD1306::LED_WrDat(unsigned char data)
{
  unsigned char i = 8;
  //LED_CS=0;
  //LED_DCH;;;
...
void SSD1306::LED_WrCmd(unsigned char cmd)
{
  unsigned char i = 8;
  //LED_CS = 0;
  //LED_DCL;;;
...
void SSD1306::LED_Set_Pos(unsigned char x, unsigned char y)
{
  LED_WrCmd(0xb0 + y);
...
void SSD1306::LED_Fill(unsigned char bmp_data)
{
  unsigned char y, x;
...
void SSD1306::LED_DLY_ms(unsigned int ms)
{
  unsigned int a;
  while (ms)
  {
...
void SSD1306::SetAddressingMode(unsigned char d)
{
...
void SSD1306::SetStartLine(unsigned char d)
{
  LED_WrCmd(0x40 | d);  // Set Display Start Line
  // Default => 0x40 (0x00)
}

void SSD1306::SetContrastControl(unsigned char d)
{
  LED_WrCmd(0x81);      // Set Contrast Control
  LED_WrCmd(d);     // Default => 0x7F
}

void SSD1306::Set_Charge_Pump(unsigned char d)
{
...
void SSD1306::Set_Segment_Remap(unsigned char d)
{
...
void SSD1306::Set_Entire_Display(unsigned char d)
{
...
void SSD1306::Set_Inverse_Display(unsigned char d)
{
...
void SSD1306::Set_Multiplex_Ratio(unsigned char d)
{
  LED_WrCmd(0xA8);      // Set Multiplex Ratio
  LED_WrCmd(d);     // Default => 0x3F (1/64 Duty)
}

void SSD1306::Set_Display_On_Off(unsigned char d)
{
...
void SSD1306::Set_Common_Remap(unsigned char d)
{
...
void SSD1306::Set_Display_Offset(unsigned char d)
{
  LED_WrCmd(0xD3);      // Set Display Offset
  LED_WrCmd(d);     // Default => 0x00
}

void SSD1306::Set_Display_Clock(unsigned char d)
{
...
void SSD1306::Set_Precharge_Period(unsigned char d)
{
...
void SSD1306::Set_Common_Config(unsigned char d)
{
...
void SSD1306::LED_P6x8Char(unsigned char x, unsigned char y, unsigned char ch)
{
...
void SSD1306::LED_P6x8Str(unsigned char x, unsigned char y, char ch[])
{
  unsigned char c = 0, i = 0, j = 0;
  while (ch[j] != '\0')
  {
...
void SSD1306::LED_P8x16Str(unsigned char x, unsigned char y, char ch[])
{
  unsigned char c = 0, i = 0, j = 0;
  while (ch[j] != '\0')
  {
...
void SSD1306::LED_PrintBMP(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1, unsigned char bmp[])
{
  int ii = 0;
  unsigned char x, y;
  for (y = y0; y <= y1; y++)
...
void SSD1306::LED_PrintValueC(unsigned char x, unsigned char y, char data)
{
  unsigned char i, j, k;
  if (data < 0)
  {
...
void SSD1306::LED_PrintValueI(unsigned char x, unsigned char y, int data)
{
...
void SSD1306::LED_PrintValueFP(unsigned char x, unsigned char y, unsigned int data, unsigned char num)
{
...
void SSD1306::LED_PrintValueF(unsigned char x, unsigned char y, float data, unsigned char num)
{
...
void SSD1306::LED_Cursor(unsigned char cursor_column, unsigned char cursor_row)
{
  if (cursor_row != 0)
  {
